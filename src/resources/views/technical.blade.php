...
@endauth
<div class="grid grid-cols-3 gap-4 m-8 md:grid-cols-5 z-0">
    @foreach($skills as $skill)
        @php
            $isCleared = auth()->check() && !is_null($current_level) && $skill->level <= $current_level - 1;
        @endphp

        <div class="flex flex-col grid-item w-24 h-24 p-2 md:p-4 rounded-lg m-auto md:w-48 md:h-48 relative
            {{ $isCleared ? 'bg-gray-200' : 'bg-green-100' }}">

            <div class="{{ $isCleared ? 'bg-gray-300' : 'bg-white' }} rounded-full w-5 h-5 p-0.5 md:w-10 md:h-10 md:p-1 absolute left-1 md:left-2 top-1 md:top-2">
                <p class="{{ $isCleared ? 'text-gray-400' : '' }} font-bold text-xs md:text-2xl text-center">
                    {{ $skill->level }}
                </p>
            </div>
            ...
            @if($isCleared)
                <div class="current-user-indicator">
                    <p class="text-center text-2xs md:text-lg p-0.5">クリア！</p>
                </div>
            @endif

            <div class="text-center flex flex-row m-auto">
                <p class="{{ $isCleared ? 'text-gray-400' : '' }} my-1 m-auto text-[10px] md:text-[20px]">
                    {{ $skill->name }}
                </p>
            </div>

            <div class="{{ $isCleared ? 'bg-green-300' : 'bg-green-600' }} border-2 border-white text-[8px] md:text-[12px] p-0.5 w-12 md:w-16 text-center rounded-full mx-auto absolute right-1 md:right-2 bottom-1 md:bottom-2">
                <button 
                    class="openModal text-white p-0.5"
    ...
    <div class="modal-content text-end m-2">
        <button id="closeModal" class=" text-2xl mx-2">×</button>
    </div>
